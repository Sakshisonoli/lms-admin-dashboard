...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (storedUserType && storedUserData) {
      try {
        ...
        setUserContext({ userType: storedUserType || 'admin', assignedBatch: null, assignedBatches: [] });
      }
    } else if (storedUserType) {
      // Has userType but no userData — set minimal context
      setUserContext({ userType: storedUserType, assignedBatch: null, assignedBatches: [] });
    } else {
      // Not logged in — set admin as default for unauthenticated state
      setUserContext({ userType: 'admin', assignedBatch: null, assignedBatches: [] });
    }
    setIsLoading(false);
  }, []);
  ...
  const updateUserContext = (newContext) => {
    ...
    localStorage.setItem('userId', newContext.userId || '');
